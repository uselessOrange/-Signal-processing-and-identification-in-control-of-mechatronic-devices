function [bestindv,bestresult,Results,meanGAesVar,stdGAesVar,BestHistory,CurrentHistory,Step,FitnessOnValidation]...
    = fGAmultiLineBest(dimms,reps,MaxSteps,P_size,n,InitialStep,P1,P2,dataset,Validation)
...
BestHistory=nan(reps,MaxSteps);
CurrentHistory=nan(reps,MaxSteps);
FitnessOnValidation=nan(reps,MaxSteps);
...
NewPopulation(k).Parameters.W1(1) = Population(Indices(ind2)).Parameters.W1(1);
NewPopulation(k).Parameters.W2(1) = Population(Indices(ind2)).Parameters.W2(1);
NewPopulation(k).Parameters.B(1) = Population(Indices(ind2)).Parameters.B(1);
