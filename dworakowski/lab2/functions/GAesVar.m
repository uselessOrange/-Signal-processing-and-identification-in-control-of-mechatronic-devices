function [bestresult,Results,meanGAesVar,stdGAesVar,BestHistory,CurrentHistory,Step]...
    = GAesVar(reps,MaxSteps,P_size,n,InitialStep,P1,P2)
...
Step(repetition,iter) = InitialStep * (1/(1+exp((iter-(MaxSteps/P1))/P2)));
...
Step(repetition,iter)*randn(size(NewPopulation(k).Parameters));
